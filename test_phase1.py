...
def test_bare_exceptions():
    ...
    code, stdout, stderr = run_command("grep -n 'except:' tars.py")
...
def test_retry_decorator():
    ...
    code, stdout, stderr = run_command("grep -n 'def retry_with_backoff' tars.py")
    if code == 0:
        print("✅ PASS: Retry decorator found")
        return True
    else:
        print("❌ FAIL: Retry decorator not found")
        return False
...
def test_validation_functions():
    ...
    if all_present:
        print("✅ PASS: All validation functions present")
        return True
    else:
...
def test_logging_setup():
    ...
    checks = [
        ...
        ("VERBOSE_MODE", "Verbose mode flag"),
        ("--debug", "Debug CLI flag"),
        ("--verbose", "Verbose CLI flag")
    ]
    ...
    if all_present:
        print("✅ PASS: Logging setup complete")
        return True
    else:
        print("❌ FAIL: Logging setup incomplete")
        return False
...
def test_help_command():
    ...
    if code == 0 and "T.A.R.S." in stdout:
...
def test_api_exception_handling():
    ...
    print("\n🧪 Test 9: ApiException Handling")
    code, stdout, stderr = run_command("grep -n 'k8s_client.exceptions.ApiException' tars.py")
    if code == 0:
        count = len(stdout.strip().split('\n'))
        print(f"✅ PASS: Found {count} ApiException handlers")
        return True
    else:
        print("❌ FAIL: No ApiException handlers found")
        return False

def test_safe_api_wrapper():
    """Test 10: Safe API wrapper exists"""
    print("\n🧪 Test 10: Safe API Wrapper")
    code, stdout, stderr = run_command("grep -n 'def safe_k8s_api_call' tars.py")
    if code == 0:
        print("✅ PASS: Safe API wrapper found")
        return True
    else:
        print("❌ FAIL: Safe API wrapper not found")
        return False
